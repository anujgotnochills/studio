        ...
        <div className="flex flex-col lg:flex-row items-center gap-5 md:gap-12 lg:gap-20">
          ...
          <div className="w-full lg:w-1/2 flex justify-center relative">
            ...
            <div className="relative group perspective-1000">
              <div className="relative w-56 h-56 sm:w-72 sm:h-72 md:w-96 md:h-96 rounded-full overflow-hidden border-4 sm:border-8 border-background shadow-2xl transition-transform duration-500 hover:scale-105 hover:rotate-3 bg-zinc-900 flex items-center justify-center">
                <span className="text-primary/70 font-bold text-sm sm:text-base md:text-lg tracking-wide uppercase">
                  Founder Photo
                </span>
              </div>
              ...
              <div className="absolute -bottom-4 -right-4 sm:-bottom-6 sm:-right-6 md:bottom-4 md:-right-8 bg-zinc-900 border-2 border-primary text-white p-3 sm:p-4 rounded-2xl shadow-xl shadow-primary/20 transform hover:-translate-y-2 transition-transform duration-300 text-center">
                <p className="font-black text-sm sm:text-base md:text-lg">{config.founderName || "Founder Name"}</p>
                <p className="font-bold text-xs sm:text-sm text-primary opacity-90">{config.founderRole || "Founder"}</p>
          ...
          <div className="w-full lg:w-1/2 flex flex-col items-center lg:items-start text-center lg:text-left mt-4 md:mt-10 lg:mt-0">
            ...
            <div className="text-sm md:text-base text-muted-foreground font-medium mb-4 leading-relaxed max-w-2xl space-y-4">
              <p>
                {config.aboutLine1 || `${config.name} is a media production house focused on premium content and creator-first execution.`} With over <strong className="text-primary text-base md:text-lg">7+ years of experience</strong>, we've grown through quality, consistency, and client trust.
              </p>
              <p>
                {config.aboutLine2 || "From branded content and corporate shoots to live event coverage, we act as invested partners rather than typical vendors."}
              </p>
              <p>
                {config.aboutLine3 || "If the work is right, it speaks for itself. Most clients stay because they know their vision will be handled flawlessly."}
              </p>
