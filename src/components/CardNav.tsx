...
const CardNav: React.FC<CardNavProps> = ({ items, className = "" }) => {
  ...
  const words = config.name.toUpperCase().split(" ");
  const brandLine1 = words.slice(0, Math.ceil(words.length / 2)).join(" ");
  const brandLine2 = words.slice(Math.ceil(words.length / 2)).join(" ");
