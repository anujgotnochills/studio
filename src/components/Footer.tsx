        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 lg:items-start">
          ...
          <div className="lg:col-span-5 flex flex-col lg:justify-start">
            ...
            <div className="contact-info flex flex-col gap-0 md:gap-3">
              ...
              <div className="contact-item flex flex-col m-0 p-0">
                ...
                <a href={`mailto:${config.email}`} className="contact-value m-0 p-0 text-white text-[16px] md:text-[22px] font-semibold leading-tight hover:text-[#a855f7] transition-colors">
                  {config.email}
                </a>
              </div>
              ...
              <div className="contact-item flex flex-col m-0 p-0 md:mt-0">
                ...
                <a href={`tel:${config.phone.replace(/\s+/g, "")}`} className="contact-value m-0 p-0 text-white text-[16px] md:text-[22px] font-semibold leading-tight hover:text-[#a855f7] transition-colors">
                  {config.phone}
                </a>
              </div>
            </div>
          ...
          <div className="lg:col-span-7 w-full flex flex-col items-center lg:items-end justify-start -mt-6 sm:-mt-1 lg:mt-0 lg:-translate-y-1">
            <div className="relative w-full lg:w-[85%] xl:w-[75%] h-[200px] sm:h-[250px] md:h-[300px] rounded-3xl overflow-hidden shadow-[0_8px_40px_rgba(0,0,0,0.5)] border border-white/10">
              <a
                href={config.mapSearchUrl || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(config.address)}`}
                target="_blank"
                rel="noopener noreferrer"
                aria-label={`Open ${config.name} location in Google Maps`}
                className="absolute inset-0 z-20"
              />
              {/* Google Maps iFrame */}
              <iframe 
                title={`${config.name} Map Location`}
                src={config.mapEmbedUrl || `https://maps.google.com/maps?q=${encodeURIComponent(config.address)}&t=&z=14&ie=UTF8&iwloc=&output=embed`} 
